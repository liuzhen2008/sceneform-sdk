...
public class MyFaceNode extends Node {
    private static final String TAG = MyFaceNode.class.getSimpleName();
    ...
    public MyFaceNode() {
        ...
        this.anchorNode.setParent(this);

        Vector3 localPosition = this.anchorNode.getLocalPosition();

        this.anchorNode.setLocalPosition(new Vector3(localPosition.x, localPosition.y + 0.1f, localPosition.z - 0.1f));

        Quaternion rotation = this.anchorNode.getLocalRotation();
        Quaternion inverse = new Quaternion(new Vector3(0.0f, 1.0f, 0.0f), 180.0f);
        rotation = Quaternion.multiply(rotation, inverse);
        this.anchorNode.setLocalRotation(rotation);
    }

    /** Create an AugmentedFaceNode with the given AugmentedFace. */
    @SuppressWarnings({"initialization"})
    public MyFaceNode(AugmentedFace augmentedFace) {
        this();
